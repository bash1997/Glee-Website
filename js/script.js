...
const promovideo = {
  title: "A creative church with authenic people",
...
const createDiv = (addClassName, elementId, src, /*nombre, series, part,*/ selection) => {
  ...
  sermonDiv.classList.add(addClassName);
  sermonDiv.setAttribute("id", elementId);
  let divVideo = document.createElement("VIDEO");
  divVideo.src = src;
  sermonDiv.append(divVideo);
  //<div class="video-info"><h5>${nombre}</h5><h6>Series: ${series}</h6><h6>Part: ${part}</h6></div>`;
  selection.append(sermonDiv);
}

const displayPromo = (src, title, addClassName, selection) => {
  let promoDiv = document.createElement("VIDEO");
  promoDiv.setAttribute("id", "promo");
  promoDiv.src = src;
  selection.append(promoDiv);
  let titleDiv = document.createElement("DIV");
  titleDiv.classList.add(addClassName);
  titleDiv.innerHTML = `<h6>${title}</h6>`;
  selection.append(titleDiv);
}

const promoSrc = promovideo["src"];
const promoTitle = promovideo["title"];
const promoClass = promovideo["class"];

//DISPLAY VIDEOS on home
...
displayFeat();
displayHomePop();
displayPromo(promoSrc, promoTitle, promoClass, promo);